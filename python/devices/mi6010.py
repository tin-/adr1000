# -*- coding: utf-8 -*-
# $Id: devices/mi6010.py | Rev 67  | 2023/10/30 05:06:48 tin_fpga $
...
class dcc_bridge():
    ...
    def __init__(self,gpib,reflevel,name):
        ...
        if cfg.get('teckit', 'interface') == 'gpib':
            self.inst = Gpib.Gpib(0, self.gpib, timeout = 320) # GPIB link
        elif cfg.get('teckit', 'interface') == 'vxi':
...
#        self.inst.write("L\r")
    ...
    def check_stby(self):
        ...
        data_str = self.inst.read()
        print ("\033[s\033[6;105H Q", end=''),
        print (data_str, data_stb, end='')
        while (data_str[1] == "s"):
            print ("\033[s\033[7;105Hwaiting for standby to go ON, end=''")
            time.sleep(1)
            data_stb = self.inst.read_stb()
            print ("\033[s\033[7;105H Q= ", end=''),
            print (data_str, data_stb)
            #data_stb = self.inst.read_stb()
            data_str = self.inst.read()
            print ("\033[s\033[8;105H RD: ", end=''),
            print (data_str, data_stb)

    def check_read(self):
        data_stb = self.inst.read_stb()
        data_str = self.inst.read()
        print ("\033[s\033[9;105HDG ", end=''),
        print (data_str, data_stb)
        while (data_str[1] == "s"):
            data_stb = self.inst.read_stb()
            data_str = self.inst.read()
    ...
    def read_data(self,cmd):
        ...
        try:
            ...
            print ("\033[s\033[%d;105H\033[32;1m ERR: " % (22+err_cnt), end=''),
            print (data_str),
            print ("\033[u")
            err_cnt = err_cnt + 1
            if (err_cnt >= 40):
...
#        self.inst.write("L\r")
    ...
    def measure(self):
        ...
        self.wait_ready()
        value = self.read_data(" ")
        print ("\033[s\033[36;1m\033[20;105H DCC READING:", end=''),
        print (value, end=''),
        print ("\033[u\033[39;0m", end='')
        return value
