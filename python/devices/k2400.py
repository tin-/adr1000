# -*- coding: utf-8 -*-
# $Id: devices/k2400.py | Rev 63  | 2022/08/12 05:13:29 tin_fpga $
...
class smu_meter():
    ...
    def __init__(self,gpib,reflevel,name):
        self.gpib = gpib
        print ("\033[8;5H \033[0;31mGPIB[\033[1m%2d\033[0;31m] : Keithley 2400\033[0;39m" % self.gpib)
        if cfg.get('teckit', 'interface') == 'gpib':
...
#        #kei.write("READ?")
    ...
    def get_data(self):
        ...
        dstr = data.split(",")
        voltage = float(dstr[0])
        current = float(dstr[1])
        #print ("\033[38;100H", voltage, current, dstr)
        if (self.status_flag):
            self.data = data#(data - 0.75) / 0.01 # Preamp A = 1000
        return voltage,current

    def get_volt_data(self):
        self.status_flag,self.data = self.read_raw_data("READ?")
        self.volts = self.data#.split(",")[0]
        if (self.status_flag):
            self.data = data#(data - 0.75) / 0.01 # Preamp A = 1000
