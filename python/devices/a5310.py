# -*- coding: utf-8 -*-
# $Id: devices/a5310.py | Rev 67  | 2023/10/30 05:06:48 tin_fpga $
...
class tecm_meter():
    ...
    def __init__(self,gpib,reflevel,name):
        ...
        self.inst = serial.Serial(port='/dev/ttyUSB0', baudrate = 38400, parity=serial.PARITY_NONE, timeout=0.1)
    ...
    def cfg_temp(self):
        self.inst.write("TEC:T 30.0\r".encode()) #set temp
        time.sleep(0.8)
        self.inst.write("TEC:OUT 1\r".encode())
        time.sleep(0.8)
        tec_rtd = 30.0

    def off_temp(self):
        self.inst.write("TEC:OUT 0\r".encode())
        time.sleep(0.8)
    ...
    def set_tmp(self,tmp):
        string = (float(tmp) - 0.02)
        global cnt
        #self.inst.write(":DISP:WIND2:TEXT:DATA \"SV %5.3f C, STEP %6d \";STAT ON;" % (tmp, cnt))
        strg = ("TEC:T %2.3f\r" % string).encode()
        print ("\033[s\033[3;3H Setting %s\033[u" % strg)
        self.inst.write(strg)
        time.sleep(0.8)
    ...
    def get_data(self):
        ...
        #self.inst.clear()
        time.sleep(0.001)
        self.status_flag,data = self.read_data("TEC:ITE?\r".encode())
        tec_curr = float(data)
        if (self.status_flag):
            self.data = data
        time.sleep(0.001)
        self.status_flag,data = self.read_data("TEC:T?\r".encode())
        if (self.status_flag):
            self.data = data
        tec_rtd = float(data)
